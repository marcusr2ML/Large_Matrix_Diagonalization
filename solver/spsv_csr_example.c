...
int main(void) {
    ...
    float     hA_values[]     = { 1.0f, 2.0f, 3.0f, 4.0f, 5.0f,
                                  6.0f, 7.0f, 8.0f, 9.0f };
    float     hX[]            = { 19.0f, 2.0f, 0.0f, 4.0f };
    float     hX2[]           = { 2.944f, 0.197f, 3.332f, 0.38f };
    float     hY[]            = { 19.0f, 2.0f, 0.0f, 4.0f };
    float     hY2[]           = { 2.944f, 0.197f, 3.332f, 0.38f };
    float     hY_result[]     = { 63.0f, 2.0f, -28.0f, 4.0f };
    float     hY2_result[]    = { 0.46f, 0.197f, 0.672f, 0.38f };
    float     alpha           = 1.0f;
    //--------------------------------------------------------------------------
    // Device memory management
    int   *dA_csrOffsets, *dA_columns;
    float *dA_values, *dX, *dY, *dX2, *dY2;
    ...
    CHECK_CUDA( cudaMalloc((void**) &dX2,        A_num_cols * sizeof(float)) )
    CHECK_CUDA( cudaMalloc((void**) &dY2,        A_num_rows * sizeof(float)) )
    ...
    CHECK_CUDA( cudaMemcpy(dY, hY, A_num_rows * sizeof(float),
                           cudaMemcpyHostToDevice) )
    CHECK_CUDA( cudaMemcpy(dX2, hX2, A_num_cols * sizeof(float),
                           cudaMemcpyHostToDevice) )
    CHECK_CUDA( cudaMemcpy(dY2, hY2, A_num_rows * sizeof(float),
                           cudaMemcpyHostToDevice) )
    //--------------------------------------------------------------------------
    // CUSPARSE APIs
    cusparseHandle_t     handle = NULL;
    cusparseSpMatDescr_t matA;
    cusparseDnVecDescr_t vecX, vecY, vecX2, vecY2;
    void*                dBuffer    = NULL;
    size_t               bufferSize = 0;
    ...
    CHECK_CUSPARSE( cusparseCreateDnVec(&vecX, A_num_cols, dY, CUDA_R_32F) )
    // Create dense vector y
    CHECK_CUSPARSE( cusparseCreateDnVec(&vecY, A_num_rows, dY, CUDA_R_32F) )
    // Create dense vector X2
    CHECK_CUSPARSE( cusparseCreateDnVec(&vecX2, A_num_cols, dY2, CUDA_R_32F) )
    // Create dense vector y2
    CHECK_CUSPARSE( cusparseCreateDnVec(&vecY2, A_num_rows, dY2, CUDA_R_32F) )
    ...
    CHECK_CUSPARSE( cusparseSpSV_analysis(
                                handle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                &alpha, matA, vecY, vecY, CUDA_R_32F,
    ...
    CHECK_CUSPARSE( cusparseSpSV_bufferSize(
                                handle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                &alpha, matA, vecY2, vecY2, CUDA_R_32F,
                                CUSPARSE_SPSV_ALG_DEFAULT, spsvDescr,
                                &bufferSize) )
    ...
    CHECK_CUDA( cudaMemcpy(hY2, dY2, A_num_rows * sizeof(float),
                           cudaMemcpyDeviceToHost) )
    int correct = 1;
    ...
    correct = 1;
    for (int i = 0; i < A_num_rows; i++) {
        if (hY2[i] - hY2_result[i] > 5e-7) { // direct floating point comparison is not
            correct = 0;               // reliable
            printf("i = %d: diff %f\n", i, hY2[i] - hY2_result[i]);
            break;
        }
    }
    if (correct)
